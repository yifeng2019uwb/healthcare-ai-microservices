...
MAVEN_CMD="mvn"
JAVA_VERSION="17"
SERVICES=("shared" "auth-service" "gateway" "patient-service" "provider-service" "appointment-service" "ai-service")
...
if [ "$SERVICE" = "all" ]; then
    case "$COMMAND" in
        "build")
            ...
            echo "✅ shared built"
            echo ""
            for service in auth-service gateway patient-service provider-service appointment-service ai-service; do
        ...
        "test")
            ...
            echo ""
            for service in shared auth-service gateway patient-service provider-service appointment-service ai-service; do
                if [ -f "$service/pom.xml" ]; then
